...
def lambda_handler(event, context):
    ...
    for year in year_options:
        for season_type in season_types:
            ...
            file_key = f"nst/raw/{year}/{season_type}/nst_{season_type}_{year}_goalies.csv"
            
            s3.put_object(Body=csv_data, Bucket=os.environ["BUCKET_NAME"], Key=file_key)
            
            files_written.append(f"nst/{season_type}/{year}/nst_{season_type}_{year}_goalies.csv")
